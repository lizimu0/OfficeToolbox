...
| 批量替换 | 在多个 docx/xlsx/pptx 中按规则查找替换,默认替换前自动备份原文件 |
| Excel 合并 | 多个 xlsx 合并为一个:每个文件一个工作表,或按行追加(表头相同) |
| 模板生成 | Word 模板 + Excel 数据源,每行数据生成一份文档(合同、通知书等) |
...
所有文件列表都支持拖入文件或整个文件夹(自动递归扫描 Office 文件);常用路径会自动记忆,下次打开自动回填;任务完成后弹出系统通知。
